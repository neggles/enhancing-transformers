...
from .layers import (
    ViTDecoder as Decoder,
    ViTEncoder as Encoder,
)
from .quantizers import GumbelQuantizer, VectorQuantizer
from enhancing.utils.general import initialize_from_config
...
class ViTVQ(pl.LightningModule):
    ...
    def training_step(
        self, batch: Tuple[Any, Any], batch_idx: int, optimizer_idx: int = 0
    ) -> torch.FloatTensor:
    ...
    def validation_step(self, batch: Tuple[Any, Any], batch_idx: int) -> Dict:
        ...
        self.log(
            "val/rec_loss", rec_loss, prog_bar=True, logger=True, on_step=True, on_epoch=True, sync_dist=True
        )
        self.log(
            "val/total_loss", aeloss, prog_bar=True, logger=True, on_step=True, on_epoch=True, sync_dist=True
        )
    ...
    def configure_optimizers(self) -> Tuple[List, List]:
        ...
        if hasattr(self.loss, "discriminator"):
            optimizers.append(
                torch.optim.AdamW(
                    self.loss.discriminator.parameters(), lr=lr, betas=(0.9, 0.99), weight_decay=1e-4
                )
            )
...
class ViTVQGumbel(ViTVQ):
    def __init__(
        ...
        super().__init__(
            image_key, image_size, patch_size, encoder, decoder, quantizer, loss, None, None, scheduler
        )

        self.temperature_scheduler = (
            initialize_from_config(temperature_scheduler) if temperature_scheduler else None
        )
