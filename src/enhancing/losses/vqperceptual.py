...
from enhancing.losses.layers import (
    StyleDiscriminator,
    conv2d_gradfix,
    hinge_d_loss,
    least_square_d_loss,
...
class VQLPIPS(nn.Module):
    ...
    def forward(
        ...
        nll_loss = (
            self.loglaplace_weight * loglaplace_loss
            + self.loggaussian_weight * loggaussian_loss
...
class VQLPIPSWithDiscriminator(nn.Module):
    ...
    def calculate_adaptive_factor(
        self, nll_loss: torch.FloatTensor, g_loss: torch.FloatTensor, last_layer: nn.Module
    ) -> torch.FloatTensor:
            ...
            if do_r1:
                with conv2d_gradfix.no_weight_gradients():
                    (gradients,) = torch.autograd.grad(
                        outputs=logits_real.sum(), inputs=inputs, create_graph=True
                    )
