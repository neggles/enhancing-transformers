...
class DataModuleFromConfig(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        self.datasets = dict(
            (k, initialize_from_config(self.dataset_configs[k])) for k in self.dataset_configs
        )
    ...
    def _val_dataloader(self):
        return DataLoader(
            self.datasets["validation"], batch_size=self.batch_size, num_workers=self.num_workers
        )
